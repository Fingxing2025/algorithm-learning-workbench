...
更新日期：2026-07-15  
审计基线：`f0017a0 feat: enrich the workbench visual system`  
源码版本：`0.1.2` 开发快照  
产品阶段：核心功能闭环，尚未达到公开正式发布标准
...
| 数据可靠性         | 可用但不完整 |        65% | 有 migration、事务和操作备份，但没有完整导出/恢复与备份管理            |
| AI Provider 稳定性 | 可用         |        75% | 五类协议已接入，跨供应商兼容矩阵和统一 JSON 修复仍不完整               |
| UI 与交互          | 较完整       |        82% | 双主题、滚动、代码主题和核心状态已覆盖，布局记忆与可访问性仍需系统验收 |
| 性能与大型工作区   | 未充分证明   |        65% | 有虚拟树和上下文上限，但没有大型工作区基准与增量相似度索引             |
| 测试与工程质量     | 良好         |        85% | 43 项单元/组件测试、22 项真实 Electron E2E 当前通过                    |
...
| 模板入库      | 粘贴/选择源码、手动字段、中英文 AI 补全、冲突逐字段确认、安全路径生成  | `create-template-dialog.tsx`、`template-intake.spec.ts`           |
...
| 题目 AI 分析  | 文本/图片、视觉任务路由、草稿编辑、确认后事务入库                      | `problem-analysis-service.ts`、`problem-analysis.spec.ts`         |
...
| AI JSON 稳定性 | 文件计划支持围栏 JSON、外层说明、常见 envelope 和一次修复   | 题目分析仍使用较严格的单次 JSON 解析；缺统一结构化输出、重试和可下载诊断              |
| Provider 兼容  | 五类 Adapter 都有实现                                       | 自动化重点验证 OpenAI/Anthropic；Gemini、Ollama、Responses 变体矩阵不完整             |
| 错误处理       | 区分鉴权、模型、限流、网络、能力错误                        | 缺统一重试策略、Retry-After、取消、离线检测、长任务超时后的恢复入口                   |
...
- 当前共有 4 个顺序 migration：初始化、题目关联、AI Provider、模板管理。
- 领域 schema 包含工作区、模板、模板元数据、题目、图片、关联、Provider、任务路由、文件计划、执行记录和应用状态。
- 已有 migration E2E 验证旧阶段数据库原位升级，不依赖删除数据库。
- 尚无完整数据库快照导出和恢复校验；这是后续最高优先级。

## 5. 工程规模与可维护性

- 受版本控制的 TypeScript/TSX/CSS/SQL 文件约 106 个，合计约 16,869 行。
- `App.tsx` 约 1,150 行。
- `template-management-service.ts` 约 978 行。
- `ai-provider-workspace.tsx` 约 716 行。
- `problem-workspace.tsx` 约 679 行。
- `problem-analysis-dialog.tsx` 约 571 行。
...
| Prettier check                     | 通过                                              |
| Vitest                             | 16 个文件，43 项通过                              |
| `npm run test:e2e`                 | 22 项通过，1 项打包入口测试按条件跳过             |
| `npm audit --audit-level=moderate` | 0 个漏洞                                          |
| Renderer 生产构建                  | 通过；主入口约 130 kB，CodeMirror 延迟块约 386 kB |
| 亮暗/紧凑截图                      | 已由 E2E 重新生成并抽查                           |

说明：打包入口 smoke test 需要先生成 `release/mac-arm64` 目录包并设置 `PACKAGED_APP_PATH`，因此常规 E2E 中跳过是预期行为。历史上已有 1 项该测试通过的记录，但任何新发布候选都必须重新运行。
...
1. V2 全量数据导出、校验和恢复，覆盖 SQLite、题目图片、Provider 非密钥配置、模板元数据和文件计划记录；密钥必须单独处理并默认不导出。
2. 异常退出恢复：检测 SQLite/WAL 状态、孤立图片、`.trash`、残留备份和未完成文件执行，并提供只读诊断与人工确认修复。
...
1. 统一题目分析、模板分类和文件计划的 JSON 提取、schema 校验、有限修复与可操作错误。
2. 建立 Provider 兼容矩阵，至少覆盖五类协议的成功、401/403、404 模型、429、5xx、超时、无效 JSON、超大响应和流式中断。
3. 为文件计划执行与回滚增加故障注入单元/集成测试，覆盖第 N 步失败、磁盘满、目标占用、外部修改和数据库提交失败。
4. 实现面板拖动与尺寸记忆；补齐小窗口、200% 缩放、长标题、长路径、长题面和超多标签状态。
5. 做全页面键盘和屏幕阅读器审计，增加状态播报和稳定的焦点回归测试。
6. 拆分超大组件和服务，避免在同一提交混入新功能与视觉重构。
7. 同步 README、用户指南、发布记录、CHANGELOG 和版本号，建立单一发布事实来源。
...
4. AI 请求 Token/成本提示、发送内容预览和本地隐私审计。
...
- AI 文件计划上下文最多取前 250 个模板，源码片段总量最多约 120,000 字符。
- 题目 AI 推荐最多取前 250 个模板，目录上下文最多约 60,000 字符。
