...
- 更新日期：2026-07-16
- Session A 第三切片基线：`219ce94 feat: add atomic data restore rollback`
- 本次交接提交：本文所在提交（`feat: add safe backup lifecycle governance`）
- 源码版本：`0.1.2` 开发快照
...
当前状态：**Session A：数据可靠性与恢复的三个纵向切片已完成；下一主线为 Session B：AI 稳定性与兼容矩阵**。

Session A 已交付三条可运行纵向切片：第一切片完成备份 ADR、版本化数据管理契约、只读一致性诊断、`.awb-backup` 目录备份包导出、全包 SHA-256 验证、损坏包拒绝和只读恢复预览；第二切片开放恢复执行、恢复前自动预备份以及 SQLite/userData 事务式恢复和故障回滚；第三切片补齐备份保留建议、空间统计、异常残留保护、逐项清理预览、应用隔离区和可撤销回滚。
...
10. 已完成：残留 `.restore-*.tmp`、未完成隔离目录和其他无法证明安全的项只读报告并保护，不从清理入口移动。
...
当前工作的重心应从“继续增加页面和功能”切换为“把已有产品做成可放心发布和长期使用的工具”。V2 数据的导出、验证、恢复、保留建议和可撤销清理已经形成主要闭环；接下来的最高价值缺口是 AI 协议兼容稳定性、异常恢复目录的人工修复流程、真实 Windows 验收和 macOS/Windows 代码签名。
...
| 数据可靠性         | 主要闭环完成 |        88% | 已有可验证导出/恢复、保留建议、隔离、撤销和故障回滚；不开放永久删除，异常恢复目录只保护不自动修复 |
...
| 测试与工程质量     | 良好         |        92% | 86 项 Vitest、34 项常规真实 Electron E2E 当前通过                                                 |
...
- 当前分支：`main`。
- 本切片阶段前 HEAD：`219ce94`；本交接完成后以本文所在提交为新基线。
...
| 模块           | 当前能力                                                                       | 仍缺少的细节                                                                             |
| -------------- | ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| V2 数据保护    | 可验证导出/恢复、原子回滚、空间统计、保留建议、逐项隔离和撤销                  | 定时备份、隔离区永久清空和异常恢复目录的人工修复入口                                     |
...
- 隔离区永久清空与异常 `.restore-*.tmp` 的人工恢复工具；当前版本选择可撤销优先，不提供不可逆删除。
- 自动更新和更新回滚策略。
...
| 数据隔离区                                            | `userData/data-management-quarantine/` | `v1` 清单、完成标记和可撤销操作；不永久删除          |
...
- 数据管理页已提供 SQLite 快照导出、全包校验、恢复预览、恢复前自动备份、恢复执行、生命周期统计、隔离和撤销；第三切片未引入数据库 schema migration。
...
| Prettier check                     | 通过                                                                              |
| Vitest                             | 23 个文件，86 项通过                                                              |
| 模板入库 Electron E2E              | 6 项通过；覆盖默认全选、无 AI、跳过、改名、覆盖备份与 AI 批量导入                 |
| `npm run test:e2e`                 | 34 项通过，1 项打包入口测试按条件跳过                                             |
| 数据管理 Electron E2E              | 5 项通过；覆盖导出/恢复、生命周期隔离撤销和两类故障回滚                           |
...
| 亮暗/紧凑截图                      | 已由 E2E 重新生成并抽查；数据管理/恢复/生命周期均补齐 1440×900、1280×720 亮暗截图 |
...
1. 异常退出后的人工修复：当前已识别并保护 `.tmp`、`.trash`、未完成恢复目录和无记录备份，但 `.restore-*.tmp` 仍只报告，不提供自动或半自动修复。
2. macOS Developer ID 签名与 notarization；Windows Authenticode 和真实 Windows 安装/升级/卸载验证。
3. 从全新 `userData` 和已有 V2 数据各跑一次发布候选验收，确认安装包不含个人数据。
...
状态：三个纵向切片已完成；不可逆永久清空和异常恢复目录人工修复不在当前已开放入口内。
