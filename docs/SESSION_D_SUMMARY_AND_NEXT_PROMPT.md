...
17. 本文所在提交：同步关闭控件永不禁用、重新打包与最终交接证据
...
本轮从源码提交 `3b97ae6` 重新执行 `npm run package:dir`，并单独通过全新 userData 与已有 V2 工作区重启的 2 项 packaged smoke。Session C 的旧候选摘要没有被当作本轮目录包证据；macOS 正式签名/公证状态仍未完成。
