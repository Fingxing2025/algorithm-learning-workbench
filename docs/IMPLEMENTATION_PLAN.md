...
## 模板分类语义与证据复核（2026-09-14）

- 建立稳定 ID 的 3–4 级 canonical taxonomy；别名按集合解析，`Tarjan`、`BFS`、`DFS` 等多义名称不能覆盖逐文件判断，`C++`、`C#` 等符号在规范化时保留。
- 单份入库、批量入库和总体文件 AI 共用 taxonomy、安全相对路径与源码证据契约。跨文件协调按 canonical family 分组，不再以共享父目录 ID 或路径强制统一不同算法族。
- 长源码按带行号的函数/代码块上下文采样并记录覆盖区间；逐字引用必须落在已发送范围，注释或字符串命中不算实现证据。请求在最终序列化后执行输入预算检查，超限时在 Provider 调用前失败。
- 全局 1,200 字摘要只保存为初步提议；逐文件详细判断保留自己的分类、路径、复杂度、证据和版本历史。冲突、覆盖不足、低置信度、通用类别、相近候选或新增类别提议统一标记 `needsReview`。
- 当前创建/批量草稿支持人工确认锁定；重新生成会跳过锁定项，修改源码或目标路径会解除锁定，关闭窗口后的迟到响应不能恢复草稿。锁定仅覆盖当前会话草稿，跨重启持久化留给后续 migration `0010`。
- 本切片没有新增 SQLite 字段、migration、依赖或 Provider 协议，也不读取或修改真实模板库。全新 userData、空白工作区和已有 V2 数据保持原位兼容。

验收：分类合同、taxonomy、证据校验、Main 服务、文件计划、后台进度和 Renderer 定向 Vitest 为 10 个文件/113 项通过；真实 Electron 模板入库 9/9、总体文件管理 5/5 通过。分类证据复核按 1440×900、1280×720、1024×640 亮色/深色截图检查。夹具验证解析、证据和人工复核流程，不代表真实模型分类准确率。
