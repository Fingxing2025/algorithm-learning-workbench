...
| TM-006    | 远程 Provider            | Provider 响应请求                  | 返回超大、畸形或慢响应耗尽资源               | UI 卡顿、请求失败          | 可用性                     | 增量读取、连接/响应双超时、1 MiB 硬上限、有限重试、可取消任务和流完成标记（`src/main/services/ai-provider-adapters.ts`; `docs/decisions/0016-ai-task-reliability-and-provider-compatibility.md`）         | 响应超时或流中断前供应商侧可能已经产生用量                         | 不自动重试不确定的流中断；提示用户检查供应商任务与用量后手工决定                                        | 统计阶段、字节、耗时与错误码，不记录响应正文                              | 中         | 低              | 低       |
| TM-007    | 发布链攻击者             | 用户下载未签名或来源不明产物       | 替换 DMG/ZIP/NSIS 或植入构建产物             | 本地代码执行、全部资产失陷 | 全部运行时资产             | 精确候选选择、SHA-256、SBOM、隐私/架构验证、preview/signed 失败关闭、ASAR、hardened runtime（`scripts/release/`; `docs/decisions/0018-release-candidate-pipeline-and-platform-evidence.md`）              | 当前无 Developer ID 签名、公证和已验证发布渠道；Windows 未实机验证 | 公布前配置 macOS Developer ID+notarization、Windows Authenticode；完成 Windows 实机验收并从可信渠道发布 | CI 保存签名、staple/Gatekeeper、hash、SBOM 与验收证据；发布后验证下载样本 | 中         | 高              | 中       |
| TM-008    | 依赖或 CI 供应链         | 恶意依赖版本、Action 或凭证        | 在安装/构建阶段执行代码并污染产物            | 发布包被植入               | 源码、构建产物             | package-lock、只读 CI、`npm ci`、audit、Action commit SHA、分平台构建与 CycloneDX SBOM（`.github/workflows/quality.yml`; `.github/workflows/release-candidate.yml`; `scripts/release/generate-sbom.mjs`） | 尚未配置 artifact attestation/provenance 与签名发布环境            | 启用受保护环境、最小 secrets、artifact attestation、Dependabot/CodeQL；审查 lockfile 和 Action SHA 更新 | Dependabot/CodeQL/审计告警，核对候选元数据与可信下载样本                  | 低         | 高              | 低       |
...
| `.github/workflows/release-candidate.yml`          | 候选平台、签名 secrets 与证据上传  | TM-007, TM-008                 |
| `scripts/release/`                                 | 预检、摘要、SBOM、签名与隐私验证   | TM-007, TM-008                 |
