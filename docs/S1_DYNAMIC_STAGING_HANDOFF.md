...
以及重启恢复和用户数据保护。S3 分类体系与 S4 持久化分类确认不属于原始独立切片。
...
暂存和恢复备份会增加磁盘占用。组合复核发现的跨进程互斥已由 S1 在激活边界补齐（参见 ADR-0038 后续章节）；
S4 负责分类确认持久化。本切片另有进程内任务追踪、会话锁、版本和哈希校验。AI 草稿是内存态，重启后需重新生成审查计划；
已持久化暂存项可继续使用。
