...
- canonical taxonomy 的稳定 ID、3–4 级路径、别名集合和路径安全校验必须由合同测试锁定；共享 `basic`、`dp` 等父目录的不同算法族不得互相覆盖。同族分歧只能标记复核，不能按最高置信度强制统一。
- 分类引用必须逐字匹配已发送源码的行号范围。纯注释、字符串或头文件名称不足以证明实现；长文件必须报告已覆盖和遗漏行数，不能把头尾截断描述为完整源码。
- 全局提议与逐文件判断都必须保留版本和来源。详细判断不得被全局摘要覆盖；冲突、部分覆盖、低置信度、通用/未知类别、相近候选、新分类提议和独立多算法目标必须进入 `needsReview`。
- 计划中的移动/重命名若缺少有效分类证据，必须默认不选并显示“需复核”。UI 文案使用“源码引用已核对”，并同时说明引用核验不证明算法判断正确。
- 人工确认只锁定当前创建或批量草稿；重新生成跳过锁定项，源码或路径变化解除锁定，关闭后的迟到响应不得恢复可保存状态。跨重启锁定尚未实现，不得按已持久化验收。
- 真实模型准确率不由 mock 夹具证明。夹具必须包含真实编写的源码、误导注释、畸形响应、同父目录跨族回归和长文件覆盖边界。
- 定向 Vitest：10 个文件/113 项通过。真实 Electron：`template-intake.spec.ts` 9/9、`file-management.spec.ts` 5/5 通过；相关流程同时覆盖原子导入、备份/撤销、跨工作区失效记录清理和关闭后的迟到响应。
- 截图验收：`s3-evidence-light-1440x900.png`、`s3-evidence-light-1280x720.png`、`s3-evidence-light-1024x640.png` 及对应深色截图均无页面级横向溢出，证据、覆盖、复核原因和锁定状态可见。
- 本切片无 schema/migration、Provider 配置、发布脚本或真实模板文件变更；未重新打包，未在真实 Windows 主机运行。
