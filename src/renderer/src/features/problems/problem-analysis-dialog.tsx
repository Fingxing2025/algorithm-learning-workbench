...
import { ProblemAnalysisRelations, type RelationDraft } from './problem-analysis-relations'
          ...
          <form className="flex min-h-0 flex-1 flex-col" onSubmit={event => void confirm(event)}>
            <div className="grid min-h-0 flex-1 gap-4 p-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(380px,0.95fr)]">
              ...
              <section className="min-h-0 space-y-4 overflow-y-auto rounded-2xl border border-border bg-surface-subtle/45 p-4">
                ...
                <ProblemAnalysisRelations
                  availableManualTemplates={availableManualTemplates}
                  onAddManualTemplate={addManualTemplate}
                  onRemoveRelation={removeRelation}
                  onSearchQueryChange={setTemplateQuery}
                  onSelectCandidate={(templateId, selected) =>
                    setSelectedCandidates(current => {
                      const next = new Set(current)
                  ...
                  onSelectManualTemplate={setSelectedManualTemplateId}
                  onUpdateRelation={updateRelation}
                  relationDrafts={relationDrafts}
                  selectedCandidates={selectedCandidates}
                  selectedManualTemplateId={selectedManualTemplateId}
