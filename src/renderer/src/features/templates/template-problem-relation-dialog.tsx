  ...
  return (
    <Dialog.Root onOpenChange={onOpenChange} open={open}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[60] w-[min(540px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border border-border bg-panel shadow-2xl outline-none">
          <header className="flex items-start border-b border-border px-5 py-4">
            ...
            <div className="min-w-0">
              <Dialog.Title className="truncate text-sm font-semibold">关联题目</Dialog.Title>
              <Dialog.Description className="mt-1 text-xs text-muted-foreground">
                将“{template.name}”关联到一道已有题目；题目和模板都不会被复制或移动。
              </Dialog.Description>
            </div>
            ...
            {candidates.length === 0 ? (
              ...
              <>
                ...
                <textarea
                  ...
                  placeholder="例如：本题实际使用该模板作为基础实现。"
                  value={note}
                />
              </>
            )}
